...
void MessageLogModule::startExperimentPhases()
{
    ...
    uint32_t phase3Duration = EXPERIMENT_PHASES[2].duration_seconds * 1000;
    uint32_t phase4Duration = EXPERIMENT_PHASES[3].duration_seconds * 1000;
    ...
    if (timeSinceStart > cooldown3End) {
        // Phase 4
        LOG_INFO("Currently in Phase 4 of the experiment");
        moduleConfig.telemetry.environment_update_interval = EXPERIMENT_PHASES[3].send_interval_seconds;
    } else if (timeSinceStart > phase3End) {
        // Cooldown between Phase 3 and Phase 4
        LOG_INFO("Currently in Cooldown period between Phase 3 and Phase 4 of the experiment");
        moduleConfig.telemetry.environment_update_interval = EXPERIMENT_PHASES[2].send_interval_seconds + cooldownDuration;
    } else if (timeSinceStart > cooldown2End) {
        // Phase 3
